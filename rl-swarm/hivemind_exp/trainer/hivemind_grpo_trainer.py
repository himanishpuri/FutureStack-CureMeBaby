...
class HivemindGRPOTrainer:
    ...
    class PublishingGRPOTrainer(GRPOTrainer):
        ...
        def compute_loss(self, model, inputs, *args, **kwargs):
            ...
            if self.state.global_step % CADENCE_OF_UPDATE_STEPS == 0:
                ...
                if "responses" in self.node.outputs:
                    for i, response in enumerate(self.node.outputs["responses"]):
                        self.logger.info(f"Response #{i+1}: {response}")
                        reward = self.node.rewards[i] if i < len(self.node.rewards) else "N/A"
                        self.logger.info(f"Reward #{i+1}: {reward}")
                
                # Store outputs for final summary
                self.stage_outputs = dict(self.node.outputs)
                self.logger.info("-" * 50)
    ...
    def train_and_save(self, trainer, train_dataset):
        ...
        # Print final model outputs if available
        if hasattr(trainer, 'stage_outputs') and trainer.stage_outputs:
            self.logger.info("-" * 60)
            self.logger.info("FINAL MODEL OUTPUTS")
            self.logger.info("-" * 60)
            
            # Store stage outputs for final summary
            stage_output = {
                "stage": self.node.stage_num,
                "outputs": trainer.stage_outputs
            }
            self.all_stage_outputs.append(stage_output)
            
            for key, value in trainer.stage_outputs.items():
                if isinstance(value, str) and len(value) < 500:  # Only print reasonable sized outputs
                    self.logger.info(f"{key}: {value}")
                else:
                    self.logger.info(f"{key}: [Output too large to display]")
        self.logger.info("=" * 60)
    ...
    def print_all_stage_outputs(self):
        """Print a summary of outputs from all stages"""
        self.logger.info("\n\n" + "=" * 80)
        self.logger.info("FINAL SUMMARY OF ALL STAGES")
        self.logger.info("=" * 80)
        
        for stage_data in self.all_stage_outputs:
            ...
            if stage_num == 0 and "question" in outputs:
                self.logger.info(f"CLIENT QUESTION:\n{outputs.get('question', 'N/A')}\n")
                if "responses" in outputs and outputs["responses"]:
                    self.logger.info(f"THERAPIST RESPONSE:\n{outputs['responses'][0]}\n")
            
            # Stage 1: Comparing Therapeutic Responses
            elif stage_num == 1:
                if "responses" in outputs and outputs["responses"]:
                    self.logger.info(f"SUPERVISOR EVALUATION:\n{outputs['responses'][0]}\n")
            
            # Stage 2: Final Integration
            elif stage_num == 2:
                if "responses" in outputs and outputs["responses"]:
                    self.logger.info(f"CLINICAL DIRECTOR SYNTHESIS:\n{outputs['responses'][0]}\n")
        
        self.logger.info("=" * 80)
        self.logger.info("END OF TRAINING SUMMARY")
        self.logger.info("=" * 80)
